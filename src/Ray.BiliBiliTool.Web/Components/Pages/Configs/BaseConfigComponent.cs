    ...
    where T : BaseConfigOptions, new()
{
    ...
    protected Task LoadConfigAsync()
    {
        ...
        {
            _saveMessage = new MarkupString($"加载配置失败：{ex.Message}");
            _saveSuccess = false;
        }
        finally
        {
    ...
    protected virtual async Task HandleValidSubmitAsync()
    {
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException("无法获取数据库配置提供器");
            }
        ...
        {
            _saveMessage = new MarkupString($"保存配置失败：{ex.Message}");
            _saveSuccess = false;
        }
        finally
        {
    ...
    private MarkupString GetSaveSuccessMessage()
    {
        ...
        if (jobKey == null)
        {
            return new MarkupString("配置保存成功！");
        }

        var status = _config.IsEnable ? "已启用" : "已停用";
        return new MarkupString($"配置保存成功！<br/>{jobKey} 已{status}。");
    }
